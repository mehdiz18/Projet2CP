<?php
	...
	if(isset($_SESSION['username'])){
		...
		<head>
			...
			<title>Messagerie Service Technique</title>
		</head>
		<body>
		<div class="container mt-2 text-white">
			...
			<?php
              require "bdd.php";
              $bdd=$GLOBALS["bdd"];
				$response=$bdd->prepare("SELECT * FROM message WHERE service='service_technique'");
				$response->execute();
				...
				if($count!=0){
					...
					while($donnes=$response->fetch()){
						...
						echo "</div>";
						echo "<div class='col'>";
						echo "<a href='delete.php?id=".$donnes["id"]."&loc=service_technique' class='text-warning'>Supprimer</a>";
						echo "</div>";
						echo "</div>";
						$i=$i+1;
