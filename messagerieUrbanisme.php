  ...
  if(isset($_SESSION['username'])){
    ...
    <html lang="en">
      <head>
        ...
        <title>Messagerie Urbanisme</title>
      </head>
      <body>
      <div class="container mt-2 text-white">
        ...
        <?php
          require "bdd.php";
          $bdd=$GLOBALS["bdd"];
          $response=$bdd->prepare("SELECT * FROM message WHERE service='urbanisme'");
          $response->execute();
          $count=$response->rowCount();
          if($count!=0){
              $i=1;
              while($donnes=$response->fetch()){
	              echo "<div class=\"row documents rounded-lg\">";
	              ...
	              echo "</div>";
                  echo "<div class='col'>";
	              echo "<a href='delete.php?id=".$donnes["id"]."&loc=urbanisme' class='text-warning'>Supprimer</a>";
	              echo "</div>";
                  echo "</div>";
                  $i=$i+1;
