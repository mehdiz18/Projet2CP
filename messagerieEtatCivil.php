<?php
	...
	if(isset($_SESSION['username'])){
		...
		<head>
			...
			<title>Messagerie Etat Civil</title>
		</head>
		<body>
		<div class="container mt-2 text-white">
			<a href="adminEtatCivil.php">Retour au menu</a>
			<a href="deconnexion.php">Déconnexion</a>
			<?php
                require "bdd.php";
                $bdd=$GLOBALS["bdd"];
				$response=$bdd->prepare("SELECT * FROM message WHERE service='etat_civile'");
				$response->execute();
				...
				if($count!=0){
					...
					while($donnes=$response->fetch()){
						...
						echo "</div>";
						echo "<div class='col'>";
						echo "<a href='delete.php?id=".$donnes["id"]."&loc=etat_civil' class='text-warning'>Supprimer</a>";
						echo "</div>";
						echo "</div>";
						$i=$i+1;
